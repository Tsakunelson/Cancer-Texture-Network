...
# The function parse takes as xml file as input, with the region and vertices coordinates of that input, for a sample whole slide images.
# The fuction parses throu the coordinates of every region and returns and array of these coordinates.
# 
...
def parse(inputfile,regions, vertices):
    ...
    Output: return an array of connected points for each region [Y,X,Z] 
    """
    ...
    for region in root.iter(regions):
        ...
        for vertex in region.iter(vertices):
            ...
            coordinates.append(temp)#(map(int, )))
...
# Get_bounds function takes the patches as input, extract the region of interest, then save it to the destination directory 
#root.attrib
def get_bounds(patches, slide_location):
    ...
    for patch in values:
        ...
        #identify if annotation is a circle or not     
...
def get_patches(img, dest_dir, slide_counter, patch_counter):
   ...
   offset = (50, 50)
   counter = 0
   img_score = np.copy(img)
   #df = pd.DataFrame(columns=["I","J","Counter","Image"])
   results = pd.read_csv("./md_HE_heatmap_results.csv")
   #for j in range(len(y_classes)):
       #df.loc[j] = [ground_truth[j],y_classes[j]]

   #df.to_csv("./"+str(i)+"md_HEresults.csv")
   #print("Saved to: "+str(i)+"md_HEresults.csv")
   
   #model = load_model('./inception_model.h5')
   #model.compile(loss= keras.losses.categorical_crossentropy, #'binary_crossentropy',
   #           optimizer=keras.optimizers.sgd(lr=0.01, momentum=0.0, decay=0.1), #'rmsprop',
   #           metrics=['accuracy'])
   ...
   for i in xrange(int(math.ceil(img_shape[0]/(offset[1]*1.0)))):
       for j in xrange(int(math.ceil(img_shape[1]/(offset[0]*1.0)))): 
           ...
           if (cropped_img.mean() > 50) and (cropped_img.mean() < 180) and (len(list(set(cropped_img.flatten())))>50):
               ...
               #print(counter)
               #img_score[offset[1]*i:min(offset[1]*i+tile_size[1],img_shape[0]),offset[0]*j:min(offset[0]*j+tile_size[0], img_shape[1])] =(0,0,results.iloc[counter]["probability"]*271)
               x = image.img_to_array(cropped_img)
               #label = model.predict(x)
               #print(label) 
               
               row = math.ceil(np.min([4*i,min(4*i+tile_size[1],img_shape[0])]))
               column = math.ceil(np.min([4*j,min(4*j+tile_size[0],img_shape[1])]))
               for m in range(int(299/10)):
                   for n in range(int(299/10)):
                       img_score[int(row*m):int(row*m+5),int(column*n):int(column*n+5)] =(0,0,results.iloc[counter]["probability"]*271)
               
               counter = counter + 1
   #df.to_csv("./heat_map.csv")
   cv2.imwrite("heat.png", img_score) 

   #import seaborn as sns
   #print(img_score.shape)
   #sns_heat = sns.heatmap(img_score, annot=True, linewidths=.5)
   #sns_heat.savefig("output.png")   
# #This is for collecting good nuclei cells
# 1. Take the minimum x value of the current slide from annotations
# 2. Take the maximum x value of the current slide from annotations 
# 3. Add a threshold to both values and extract the general region from the slide
# 4. Delete the annotated regions by replacing with white 
# 5. get 50*50 patches of good images 
# 6. Store in directory of good cells

# The combined process with function calls for a larger dataset begins here

# In[26]:
#def counting_components(connecivity,inputimage):
    #src = cv2.imread(inputimage, 0)
#    binary_map = (inputimage < 255).astype(np.uint8)
    # or whatever you prefer

#    output = cv2.connectedComponentsWithStats(binary_map,connecivity,  cv2.COLOR_GRAY2BGR )
 #   return output
...
slide_counter = 0
for temp in range(len(svs)):
    temp_xml = "./poorly_differentiated/CK/valid/"+xml[temp]
    temp_svs = "./poorly_differentiated/CK/valid/"+svs[temp]
    print(temp_xml)
    print(temp_svs)
'''
images = parse("./1003978.xml",regions = 'Region', vertices = 'Vertex')
    
positive_nuclei, good_mask, slide, values = get_bounds(images, "./1003978.svs")
    
    #negative_nuclei_roi = get_good_nuclei(mask = good_mask, slide = slide, values = values)
    #outputs = counting_components(4,negative_nuclei_roi[0])
    #print(type(outputs))
    #print(len(outputs)) 
